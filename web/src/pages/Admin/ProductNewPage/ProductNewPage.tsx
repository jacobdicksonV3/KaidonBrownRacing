...
const generateVariants = (attributes: Attribute[]): VariantRow[] => {
...
const variantLabel = (options: Record<string, string>) =>
...
const AdminProductNewPage = () => {
      ...
      variables: {
        input: {
          ...
          sizes: JSON.stringify(
            attributes.length === 1 ? attributes[0].values : variants.map((v) => variantLabel(v.options))
          ),
  ...
  const handleGenerateVariants = () => {
    ...
    // Preserve stock/sku/price from existing variants where options match
    const merged = generated.map((g) => {
      const label = JSON.stringify(g.options)
      const existing = variants.find((v) => JSON.stringify(v.options) === label)
  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="font-heading text-2xl font-bold tracking-wider text-white">New Product</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <Label>Name</Label>
                <Input value={form.name} onChange={set('name')} className="mt-1" required />
              </div>
              <div>
                <Label>Slug <span className="text-white/30">(auto if empty)</span></Label>
                <Input value={form.slug} onChange={set('slug')} className="mt-1" placeholder="auto-generated" />
              </div>
              <div>
                <Label>Base Price (AUD)</Label>
                <Input type="number" step="0.01" value={form.price} onChange={set('price')} className="mt-1" required />
              </div>
              <div>
                <Label>Category</Label>
                <Input value={form.category} onChange={set('category')} className="mt-1" required placeholder="e.g. Apparel" />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <Textarea value={form.description} onChange={set('description')} className="mt-1" rows={4} required />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Define options like Size, Colour, etc. Each attribute&apos;s values become selectable on the storefront.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {attributes.length > 0 && (
